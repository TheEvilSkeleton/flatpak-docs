...
- Flatpak: the framework for distributing desktop applications across various Linux distributions.
...
small number of freedesktop.org standards, in order to enable desktop integration
(see :doc:`conventions`).
...
- **Space efficiency**: Flatpak links identical runtime and BaseApp versions to save megabytes
  or even gigabytes worth of storage depending on the amount of applications installed.
...
- **Full control over dependencies**: Flatpak makes it easy to bundle specific
  libraries as part of the app.
